			...
			for(unsigned int j=0; j<mMap.vpKeyFrames.size(); j++)
			{
				...
				if(k.mMeasurements.count(p)){
					...
					for(unsigned int ibest=0;ibest<k.iBestPointsCount;ibest++){ //check all best points
						...
						{
							k.apCurrentBestPoints[ibest] = NULL;
							int mindist = 9999;
							int tempdist = 0;
...
//Weiss{
	...
	if (pPars->MaxKF>1)	// MaxKF<2 means keep all KFs
	{
		while (mMap.vpKeyFrames.size()>(unsigned int)pPars->MaxKF)
		{
			...
			vector<KeyFrame*>::iterator itFarthest = mMap.vpKeyFrames.begin();
...
//Weiss{
	...
	Vector<3> veccam2;
	Vector<2> cam2,cam1;
	for(std::map<MapPoint*, Measurement>::iterator it = kCurrent.mMeasurements.begin();it != kCurrent.mMeasurements.end(); it++)
	{
...
bool MapMaker::RefreshSceneDepth(KeyFrame *pKF)
{
