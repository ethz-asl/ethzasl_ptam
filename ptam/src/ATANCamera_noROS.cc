...
ATANCamera::ATANCamera(string sName, double Cam_fx, double Cam_fy, double Cam_cx, double Cam_cy, double Cam_s, double ImageSizeX, double ImageSizeY)
{
  // The camera name is used to find the camera's parameters in a GVar.
  msName = sName;
  //Weiss{
  //mgvvCameraParams = mvDefaultParams;
  mgvvCameraParams = makeVector(Cam_fx, Cam_fy, Cam_cx, Cam_cy, Cam_s);
  //GV2.Register(mgvvCameraParams, sName+".Parameters", mvDefaultParams, HIDDEN | FATAL_IF_NOT_DEFINED);
  //mvImageSize[0] = 640.0;
  //mvImageSize[1] = 480.0;
  mvImageSize[0] =ImageSizeX;
  mvImageSize[1] = ImageSizeY;
  //}
  RefreshParams();
}
